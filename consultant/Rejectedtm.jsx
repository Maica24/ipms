...
const Rejectedtm = () => {
    ...
    const fetchSubmissions = async () => {
        try {
            const response = await axios.get(`${API_BASE_URL}/api/tm-submissions-rejected`);
            console.log('✅ Fetched Rejected TM submissions:', response.data);
            setSubmissions(response.data);
            setLoading(false);
        } catch (err) {
            console.error('❌ Error fetching data:', err);
            setError('Failed to fetch Rejected Trademark submissions.');
            setLoading(false);
        }
    };
    ...
    return (
        <div className="Table-container">
            <h2>
                ...
                Rejected Submissions - Trademark
            </h2>

            {submissions.length === 0 ? (
                <div className="error-message" style={{ background: '#f0fdf4', color: '#15803d' }}>
                    <i className="bi bi-check-circle me-2"></i>
                    No rejected Trademark submissions found. All submissions are in good standing!
                </div>
            ) : (
                <>
