...
// Assuming you still want to use the general Row component for table display
        ...
        <div className="popup-overlay" onClick={onClose}>
            <div className="popup-card" onClick={(e) => e.stopPropagation()}>
                ...
                <div className="preview-section">
                    <h4><i className="bi bi-image me-2" aria-hidden="true"></i>Design Image/File</h4>
                    {project.design_image_path ? (
                        <>
                            {/* Check if it's an image or document */}
                            {/\.(jpg|jpeg|png|gif|webp|svg)$/i.test(project.design_image_path) ? (
                                <img
                                    src={`${API_BASE_URL}/uploads/${project.design_image_path}`}
                                    alt="Design"
                        ...
                        <p className="no-data">No image/file submitted.</p>
                    )}
                </div>
                    ...
                    {project.official_form_path ? (
                        <>
                            <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                                ...
                                <a
                                    href={`${API_BASE_URL}/uploads/${project.official_form_path}`}
                                    download
                            ...
                            {!pdfError ? (
                                <iframe
                                    src={`${API_BASE_URL}/uploads/${project.official_form_path}#toolbar=1`}
                                    style={{
                                        width: '100%',
                                        height: '400px',
                                        marginTop: '10px',
...
const UnderReviewid = () => {
    ...
    const fetchSubmissions = async () => {
        try {
            ...
            const response = await axios.get(`${API_BASE_URL}/api/id-submissions-under-review`);
            console.log('✅ Fetched Under Review submissions:', response.data);
    ...
    // Handler for saving checklist status only (calls the new id-checklist-update endpoint)
    const handleChecklistUpdate = async (projectId, checklistData) => {
        try {
            const response = await axios.put(`${API_BASE_URL}/api/id-checklist-update/${projectId}`, checklistData);
    ...
    // Modified handler for final decision (calls the id-review-action endpoint)
    const handleFinalAction = async (projectId, action, rejection_reason) => {
        ...
        try {
            ...
            await axios.put(`${API_BASE_URL}/api/id-review-action/${projectId}`, { action, rejection_reason });
        ...
        <div className="Table-container">
            <h2>
                <i className="bi bi-hourglass-split me-2" aria-hidden="true"></i>
                 Under Review Submissions Table
            </h2>
