...
const UtilityModel = () => {
    ...
    const fetchSubmissions = async () => {
        try {
            const response = await axios.get(`${API_BASE_URL}/api/um-submissions-new`);
            console.log('✅ Fetched NEW Utility Model data:', response.data);
            setSubmissions(response.data);
            setLoading(false);
        } catch (err) {
            console.error('❌ Error fetching data:', err);
            setError('Failed to fetch Utility Model submissions.');
            setLoading(false);
        }
    };
    ...
    const handleReceive = async (projectId) => {
        ...
        try {
            await axios.put(`${API_BASE_URL}/api/um-receive/${projectId}`);
